...
def _build_block_index(
    ...
    """Build block index for sparse attention.
    
    This function enforces a fixed pattern for each query block:
    - The first block (attention sink)
    - The main diagonal block
    - The block one diagonal below the main diagonal
    
    These three blocks are forced by setting their scores to infinity before the topk operation.
    The remaining blocks are selected based on the attention scores between query and key blocks.
    
    Args:
        query: Query tensor of shape [batch_size, num_heads, seq_len, head_dim]
        key: Key tensor of shape [batch_size, num_heads, seq_len, head_dim]
        top_k: Number of blocks to select per query block
        block_size_M: Query block size
        block_size_N: Key block size
        
    Returns:
        Block indices tensor of shape [batch_size, num_heads, num_query_blocks, top_k]
    """
    ...
    # Replace the direct assignment with diagonal_scatter
    diag_size = min(p_pool.shape[-2], p_pool.shape[-1])
    inf_diag = torch.full((batch_size, num_heads, diag_size), float('inf'), device=p_pool.device)
    
    # Set main diagonal to inf
    p_pool = torch.diagonal_scatter(p_pool, inf_diag, dim1=-2, dim2=-1)

    # Set one diagonal below main to inf
    p_pool = torch.diagonal_scatter(p_pool, inf_diag[..., :-1], offset=-1, dim1=-2, dim2=-1)
    
    # Keep the causal mask
    p_pool = p_pool.where(arange_M[None, None, :, None] >= arange_N[None, None, None, :], -torch.inf)

    # Set the first block to inf - always pick attention sinks
    p_pool[..., 0] = torch.inf

    top_k = min(top_k, context_size // block_size_N)
    # assert top_k <= context_size // block_size_N, f"top_k ({top_k}) must be <= context_size/block_size_N ({context_size // block_size_N})"
